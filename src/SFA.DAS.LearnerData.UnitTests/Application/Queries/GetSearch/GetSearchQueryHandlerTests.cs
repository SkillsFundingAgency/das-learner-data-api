...
public class GetSearchQueryHandlerTests
{
    ...
    )
    {
        ...
        repository
            .Setup(x => x.Search(query.UkPrn, query.Page, query.PageSize, query.Limit, query.Offset, query.SortColumn, query.SortDescending, query.Filter, query.ExcludeApproved, query.StartMonth, query.StartYear, query.MaxStartDate, query.ExcludeUlns, It.IsAny<CancellationToken>())).ReturnsAsync(learners)
            .Verifiable();
