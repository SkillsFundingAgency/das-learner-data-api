...
public class GetLearnerByIdCommandHandlerTests
{
    [Test, MoqAutoData]
    public async Task Handle_GetById_When_Learner_Exists(
        ...
        GetLearnerByIdCommandHandler sut
    )
    {
        repository.Setup(x => x.GetById(query.Id, It.IsAny<CancellationToken>())).ReturnsAsync(learner);
    ...
    {
        repository.Setup(x => x.GetById(query.Id, It.IsAny<CancellationToken>())).ReturnsAsync(() => null);
